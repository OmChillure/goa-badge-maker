...
function EditorPage() {
  ...
  useEffect(() => {
    ...
    // /themes writes the chosen theme to the same key, and coming back here is
    // a client-side navigation that may not remount this component — so re-read
    // when the page is shown again. `storage` covers the same edit made in
    // another tab, which by spec never fires in the tab that wrote it.
      ...
      <header className="border-b border-border/60 bg-card/60 px-5 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3">
          <a href="/" className="group">
            ...
            <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.25em] text-muted-foreground group-hover:text-foreground">
              ← ID card studio
            </p>
          </a>
        ...
        <section className="order-2 lg:order-1">
          <Tabs defaultValue="you">
            ...
            <TabsContent value="style" className="space-y-4 pt-4">
              {/* The full gallery previews each colourway on a real badge,
                  which these swatches cannot. Anchor, not <Link> — see the
                  storage re-read note on mount. */}
              <a
                href="/themes"
                className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 transition hover:border-primary"
              >
                <span
                ...
                <span className="min-w-0">
                  <span className="block text-sm font-medium">Browse all themes</span>
                  <span className="block font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                    Live badge previews · HH Goa official
                  </span>
                </span>
              </a>
